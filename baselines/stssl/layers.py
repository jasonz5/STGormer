...
class STEncoder(nn.Module):
    def __init__(self, Kt, Ks, blocks, input_length, num_nodes, droprate=0.1):
        ...
        out_len = input_length - 2 * (Kt - 1) * len(blocks) # 19-2*2*2=11
    ...
    def forward(self, x0, graph):
        ...
        ## ST block 1
        x = self.tconv11(x)    # nclv ex.nc19v  
        x, x_agg, self.t_sim_mx = self.pooler(x)  # ncq(17)v  nvc  lnv
        self.s_sim_mx = sim_global(x_agg, sim_type='cos') # vv

        x = self.sconv12(x, Lk)   # nclv
        x = self.tconv13(x)   # ncl(=15)v
        x = self.dropout1(self.ln1(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))
        
        ## ST block 2
        x = self.tconv21(x) # ncl(=13)v
        x = self.sconv22(x, Lk)
        x = self.tconv23(x) # ncl(=11)v
