...
class STResNet(nn.Module):
    def __init__(self,
        ...
        self.save_path = f"L{self.nb_residual_unit}_C{self.len_closeness}_P{self.len_period}_T{self.len_trend}/"
        self.best_rmse = 999
        self.best_mae = 999
    ...
    def forward(self, xc, xp, xt, ext):
        ...
        #external
        ext_out = self.ext_net(ext).view([-1, self.nb_flow, self.map_heigh, 
                                          self.map_width])
        # FUSION
    ...
    def train_model(self, train_loader, test_x, test_y):
        ...
        for ep in range(self.epoches):
            ...
            for i, (xc, xp, xt, ext, y) in enumerate(train_loader):
                ...
                if i % 50 == 0:
                    # print("[%.2fs] epoch %d it %d, loss %.4f"%(time.time() - start_time, ep, i, loss.item()))
                    logger.info("[%.2fs] epoch %d iter %d, loss %.4f"%(time.time() - start_time, ep, i, loss.item()))
            # print("[%.2fs] ep %d, loss %.4f"%(time.time() - start_time, ep, np.mean(epoch_loss)))
            logger.info("[%.2fs] epoch %d, loss %.4f"%(time.time() - start_time, ep, np.mean(epoch_loss)))
            ...
            logger.info("[%.2fs] epoch %d test rmse %.4f, mae %.4f" % (time.time() - start_time, ep, test_rmse, test_mae))
